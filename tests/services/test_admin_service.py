...
def test_deletar_comentario_sucesso(mocker):
    ...
    mocker.patch(
        "app.services.admin_service.AdminRepository.buscar_avaliacao",
        return_value=avaliacao_fake
    )

    mock_remover = mocker.patch(
        "app.services.admin_service.AdminRepository.remover_comentario"
    )
...
def test_aprovar_admin_sucesso(mocker):
    ...
    mocker.patch(
        "app.services.admin_service.AdminRepository.buscar_pedido_admin",
        return_value=pedido_fake
    )

    mock_aprovar = mocker.patch(
        "app.services.admin_service.AdminRepository.aprovar_pedido"
    )
...
def test_deletar_todas_avaliacoes(mocker):
    mock_deletar = mocker.patch(
        "app.services.admin_service.AdminRepository.deletar_todas_avaliacoes"
    )
...
def test_deletar_docentes_sem_vinculo(mocker):
    mock_deletar = mocker.patch(
        "app.services.admin_service.AdminRepository.deletar_docentes_sem_vinculo"
    )
